...
byte deviceCommandHandler(const byte deviceIdx, const int commandID, const boolean post, const int commandvalue) {
	...
	if (DEBUG_DEVICE_HAND) dbg.print(deviceIdx);
	if (DEBUG_DEVICE_HAND) dbg.print(" Cmd ");
	if (DEBUG_DEVICE_HAND) dbg.print(commandID);
	if (DEBUG_DEVICE_HAND) dbg.print(" Val: ");
	if (DEBUG_DEVICE_HAND) dbg.println(commandvalue);
