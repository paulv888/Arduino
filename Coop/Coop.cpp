...
void setup() {
	...
	mdevices[ARDUINO_IDX].begin( "Arduino"      ,  98, ARDUINO_IDX , (long)60*60*1000   , arduinoCallbackT  );
	...
	mdevices[RELAY_1_IDX].begin( "Light1 A4"    , 205, RELAY_1_IDX , (long)0            , NULL              );
	mdevices[RELAY_2_IDX].setPin(RELAY_2_PIN);
	mdevices[RELAY_2_IDX].setType(TYPE_DIGITAL_IO);
	mdevices[RELAY_2_IDX].begin( "Light2 A5"    , 206, RELAY_2_IDX , (long)0            , NULL              );
	...
	mdevices[AUTO_FAN_IDX].begin( "Cool 6-A2"   , 208, AUTO_FAN_IDX, (long)1*15*1000     , thermoCallbackT   );
