...
void setup() {
	...
	wdt_reset();
	if (DEBUG_MAIN || DEBUG_DEVICE_HAND || DEBUG_WEB || DEBUG_DEVICE || DEBUG_MEMORY) {
		mdevices[RELAY_3_IDX].setPin(RELAY_1_PIN);
	} else {
	...
	wdt_reset();
	// Half all these Uno bootsloader with avrdude internal clock 8mhz
	if (timer.every((long)5 * 60 * 1000, reportTimer, REPORT_10MIN) < 0) {  // 10MIN reports
		showStatus(TIMER_ERROR, ARDUINO_IDX);
	}

	if (timer.every((long)60 * 60 * 500 + (long)55 * 1000, reportTimer, REPORT_HOURLY) < 0) {  // Hourly reports
		showStatus(TIMER_ERROR, ARDUINO_IDX);
	}
	// TimerCount = 4

	if (timer.every((long)24 * 60 * 60 * 500, reportTimer, REPORT_DAILY) < 0) {  // Daily reports
		showStatus(TIMER_ERROR, ARDUINO_IDX);
	}
	// TimerCount = 5
	if (timer.every((long)1 * 15 * 500, checkTimer, CHECK_15_SEC) < 0) {  // 15 Sec checks
		showStatus(TIMER_ERROR, ARDUINO_IDX);
		//		Serial.print("ETMR");
		//		Serial.println(ARDUINO_IDX);
	}
	// TimerCount = 6
	if (timer.every((long)1 * 60 * 500, checkTimer, CHECK_1_MIN) < 0) {  // 1 Min checks
		showStatus(TIMER_ERROR, ARDUINO_IDX);
		//		Serial.print("ETMR");
		//		Serial.println(ARDUINO_IDX);
	}
